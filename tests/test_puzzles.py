...
from resolver.drivers.puzzle_drivers import DomainPuzzle, RegistrationFeePuzzle, DomainInnerPuzzle
from resolver.puzzles.puzzles import DOMAIN_PH_MOD, DOMAIN_PH_MOD_HASH, REGISTRATION_FEE_MOD, REGISTRATION_FEE_MOD_HASH, INNER_SINGLETON_MOD

seed = mnemonic_to_seed("swarm fly ability pipe decide square involve caution tonight accuse weasel zero giant comfort sword brain sister want soccer mutual control question grass impact")
...
class TestPuzzles:
    ...
    @pytest.mark.asyncio
    async def test_inner_puzzle(self):
        domain_name = "jack.xch"
        pub_key = PRIVATE_KEY.get_g1()
        m_data = [("a","a"), ("b","b")]
        reg_class = DomainInnerPuzzle(DEFAULT_CONSTANTS.AGG_SIG_ME_ADDITIONAL_DATA, DEFAULT_CONSTANTS.MAX_BLOCK_COST_CLVM,
                                      domain_name, pub_key, m_data)
